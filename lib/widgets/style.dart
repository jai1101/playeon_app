import 'package:flutter/material.dart';

const AppName = "Playeon";

const primaryColor1 = Color(0xFF50C003);
const primaryColor2 = Color(0xFF1D1D3D);
const primaryColorB = Colors.black;
const primaryColorW = Colors.white;

const backgroundColor = Color(0xFF121212);
const backgroundColorB = Color(0xFF242424);
const backgroundCardColor = Color(0xFF161822);
const cardBackgroundColor = Color(0xFF161616);
const fillColor = Color(0xFF242424);
const fillColor2 = Color(0xFFFFFFFF);
const borderColor = Color(0xFF0B0A0A);
const borderColor2 = Color(0xFF475888);
const stampColor = Color(0xFF475888);
const dividerColor = Color(0xFFE3E3E3);

const pendingColor = Color.fromARGB(255, 193, 191, 188);
const completeColor = Color(0xFF27AE60);

const textColor1 = Color(0xFFFFFFFF);
const textColor2 = Color(0xFF222222);
const textColor3 = Color(0x222222);
const textColor4 = Color(0xff1F1F1F);
const textColor5 = Color(0xff4F4F4F);
const textColor6 = Color(0xffF2F2F2);
const textColorB = Color(0xFF242424);
const textColorS = Color(0xFF828282);
const textColorE = Color(0xFFE53935);
const textColorW = Colors.white;
const textColorH = Color(0xFFBDBDBD);
const textColorP = Color(0xFFDAB898);

const verticalPadding = 0.025;
const horizontalPadding = 0.035;

const fontLight = "Inter-Light";
const fontRegular = "Inter-Regular";
const fontMedium = "Inter-Medium";
const fontBold = "Inter-Bold";
const fontSemiBold = "Inter-SemiBold";
const fontExtraBold = "Inter-ExtraBold";
